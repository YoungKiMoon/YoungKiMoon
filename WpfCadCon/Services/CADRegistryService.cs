...
{
    class CADRegistryService
    {
        public static void runReg()
        {
            CreateDemandLoadingEntriesCustom("TankProgram", "C:\\ASUS\\Wpfcad.dll", new List<string>(), new List<string>(), new List<string>(), 2, false, true);
            
        }

        public static bool RegisterForDemandLoading(Assembly assem, bool currentUser, bool startup, bool force)
        {
            ...
            foreach (Module mod in mods)
            {
                ...
                foreach (Type type in types)
                {
                    ...
                    foreach (MethodInfo meth in meths)
                    {
                        ...
                        IList<CustomAttributeData> attbs = CustomAttributeData.GetCustomAttributes(meth);
        ...
        private static bool CreateDemandLoadingEntriesCustom(
                  string appName,
                  string path,
                  List<string> globCmds,
                  List<string> locCmds,
                ...
                )
        {
            ...
            RegistryKey appk =
              hive.CreateSubKey(ackName + "\\" + "Applications",RegistryKeyPermissionCheck.ReadSubTree);
            using (appk)
            {
                // Already registered? Just return (unless forcing)
                ...
                RegistryKey rk = appk.CreateSubKey(appName,RegistryKeyPermissionCheck.ReadWriteSubTree);
                using (rk)
                {
                    rk.SetValue(
                      "DESCRIPTION", appName, RegistryValueKind.String
