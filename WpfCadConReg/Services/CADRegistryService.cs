...
namespace WpfCadCon.Services
{
    class CADRegistryService
    {
        ...
        {
            CreateDemandLoadingEntriesCustom("TankProgram", "C:\\ASUS\\Wpfcad.dll", new List<string> { "STP" }, new List<string>() { "STP" }, new List<string>(), 2, false, true);
            
        }
        public static void runRegDelete()
        {

            RemoveDemandLoadingEntries("TankProgram", false);
        }
                ...
                )
        {
            ...
            RegistryKey appk =hive.OpenSubKey(ackName + "\\" + "Applications",true);
            using (appk)
            {
                // Already registered? Just return (unless forcing)
